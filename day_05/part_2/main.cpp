...
class Ship {
    ...
    private:
        ...
        */
        ...
        /*
        *   Input: integers representing the source and destination of a crate to move, as well as number of crates
        *   Description: removes numberToMove crates from the top of the source stack, and adds them to the top
        *   of the destination stack, in the same order.
        */
        void moveCrates(int source, int destination, int numberToMove) {

            vector<char> movedCrates;

            for (int i = 0; i < numberToMove; i++) {
                movedCrates.push_back(this->stacks[source-1].back());
                this->stacks[source-1].pop_back();
            }

            for (int i = 0; i < numberToMove; i++) {
                this->stacks[destination-1].push_back(movedCrates.back());
    ...
    public:
        ...
        */
        void doInstruction(string line) {
            ...
            this->moveCrates(newInstruction.source, newInstruction.destination, newInstruction.numberToMove);
